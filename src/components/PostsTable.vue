<template>
  <div :class="$style['posts-table']">
    ...
    <component :is="tableComponent" :items="preparedPosts" :columns="columns">
        ...
        <div :class="[$style['header-item'], headerItemClass]">
          <div :class="$style['name-wrapper']">
            ...
            <Icon
              ...
              @click.native="onSortClick(column.id)" />
          </div>
        ...
        >
          ...
          <template #error="{ trigger }">
            <div :class="$style['infinite-message']">
              <div>Не удалось подгрузить данные</div>
              <button :class="$style.button" @click="trigger">Повторить</button>
            </div>
          </template>
        </InfiniteLoading>
      </template>
